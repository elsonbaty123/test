...
export const BOX_TYPES: BoxTypeConfig[] = [
  {
    id: 'mimi',
    name: 'ميمي',
    description: 'البوكس الاقتصادي - دراي فود فقط',
    includeDryFood: true,
  ...
  },
  {
    id: 'toty',
    name: 'توتي',
    description: 'دراي فود + ويت فود (كيس واحد/أسبوع)',
    includeDryFood: true,
    includeWetFood: true,
  ...
  },
  {
    id: 'qatqoot_azam',
    name: 'القطقوط الأعظم',
    description: 'دراي فود + ويت فود (3 أكياس/أسبوع) + تريتين',
    includeDryFood: true,
    includeWetFood: true,
...
export function useCatNutrition() {
  ...
  const calculateBoxPricing = useCallback((results: Results): BoxPricing[] => {
    ...
    for (const boxType of boxTypeConfigs) {
      for (const variant of BOX_VARIANTS) {
        ...
        const baseWeeklyData = results.weeklyData
  ...
  const calculateBoxPricingDirect = useCallback((): BoxPricing[] => {
    ...
    // استخدم النتائج من الـ state بدلاً من استدعاء calculateNutrition
    if (!results || !results.weeklyData || results.weeklyData.length === 0) {
      return []
    }
    ...
    return boxPricings
  }, [pricing, boxTypeConfigs, boxBuilder, foodData, weeklyPlan, catData, results, createBoxSpecificWeeklyPlan])
